...
def listCategories():
    addDir('Destaque',    'http://api.vevo.com/mobile/v2/featured/carousel.json?',   'listFeatured')
    addDir('Premieres',   'http://api.vevo.com/mobile/v1/video/list.json?ispremiere=true',        'listVideos')
    addDir('Selecionadas pelo Vevo',        '',                                                     'listStaffPicks')
    ...
    if (addon.getSetting('latitude') <> '') or (addon.getSetting('latitude') <> ''):
        ...
        addDir('Tendências',       '',                                                 'Trending', cm=cm)
        addDir('Tournes',          '',                                               'toursRightNow', cm=cm)
    addDir('Videos',       'http://api.vevo.com/mobile/v1/video/list.json',                'rootVideos')
    addDir('Artistas',            'http://api.vevo.com/mobile/v1/artist/list.json',         'rootArtists')
    ...
    if video:
        ...
        if os.path.exists(videofile):
            if status == 'failed':
                ...
                print "Reproduzindo %s do Cache" % filename
...
def HTTPDynamicCacheNFO(video,nfofile):
    ...
    except: print 'nfo falhou'

def HTTPDynamicCacheDownload(vevoID):
    print "Cacheando %s" % vevoID
    url = 'http://videoplayer.vevo.com/VideoService/AuthenticateVideo?isrc=%s' % vevoID
    data = getURL(url)
    video = demjson.decode(data)['video']
    title = video['title'].encode('utf-8')
    ...
    subfile = os.path.join(cachepath,filename+'.srt')
    if os.path.exists(videofile):
        print "Encontrado %s no cache, Adicionado para savedDB e resolvendo" % filename
        item = xbmcgui.ListItem(path=videofile) 
        xbmcplugin.setResolvedUrl(pluginhandle, True, item)
        ...
        if video_url:
            ...
            if os.path.exists(videofile):
                if dlThread.isAlive():
                    ...
                    print "Reproduzindo %s enquanto transferindo" % filename
                    item = xbmcgui.ListItem(path=videofile) 
                    xbmcplugin.setResolvedUrl(pluginhandle, True, item)
        else:
    ...
    try:
        ...
        if addon.getSetting('us_proxy_enable') == 'true':
            ...
            if addon.getSetting('us_proxy_pass') <> '' and addon.getSetting('us_proxy_user') <> '':
                print 'Usando proxy autenticado: ' + us_proxy
